...
/*
 * Standard 5x8 bitmap font - Adafruit GFX / GLCD format.
 * Each character is 5 bytes (one byte per column, left to right).
 * Bit 0 = top pixel, Bit 7 = bottom pixel (only bits 0-7 used).
 * Covers ASCII 32 (space) through 126 (~) = 95 characters.
 * Total: 95 * 5 = 475 bytes, stored in flash (.rodata).
 */
...
/*
 * Standard 8x16 bitmap font - IBM VGA / CP437 style.
 * Each character is 16 bytes (one byte per row, top to bottom).
